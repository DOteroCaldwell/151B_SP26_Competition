...
Reinforcement learning — GRPO, DPO, outcome-based reward modeling, and related alignment methods
3. External model calls, API access, and tool-augmented generation (e.g., code interpreters or calculators) are not permitted at inference time. Success requires extracting the most mathematical reasoning capability possible from a single mid-sized model through careful optimization.
...
# Dataset Description
Starter Template
The Starter Code for the competition is present at: https://github.com/brooksniu/151B_SP26_Competition. Refer to the notebook to understand our dataset and the data loading procedures.
