...
\paragraph{Phase~2 sequence length results.}
\cref{tab:phase2comparison} compares Phase~1 and Phase~2 accuracy on the 20-question validation set. Phase~2 achieves a 35.0~pp gain in overall accuracy, with the largest improvement in MCQ (+55.6~pp). The error breakdown in \cref{tab:phase2errors} shows that \texttt{FREE\_FORM\_NO\_BOX} drops from 7 to 0 and \texttt{MCQ\_NO\_VALID\_LETTER} drops from 7 to 1. This confirms that increasing the model's sequence length parameter from 2048 to 8192 tokens allows for complete chain-of-thought reasoning, which previously was being cut off before reaching the final formatted answer. The dominant remaining error type shifts to \texttt{FREE\_FORM\_WRONG\_ANSWER} (5 errors), indicating that the model now has sufficient space to format its responses correctly but still arrives at an incorrect answer in some cases.
...
Several observations follow from these results. First, MCQ accuracy at baseline is catastrophically low (10.7\% on the full set, 11.1\% on the 20-question sample), and the cause is almost entirely formatting: 87.5\% of MCQ mini-baseline errors are \texttt{MCQ\_NO\_VALID\_LETTER}. Once the sequence length is increased to 8192 tokens, MCQ accuracy jumps to 66.7\%, suggesting that the underlying mathematical reasoning for MCQ was previously present but truncated before the final answer could be emitted. Second, single-part free-form accuracy is unchanged at 75.0\%, indicating that shorter problems could already complete their reasoning within the 2048-token limit. Third, multi-part free-form accuracy more than triples from 14.3\% to 42.9\%, further demonstrating that complex, multi-step problems require the additional output space to reach a formatted conclusion. These results suggest that the remaining gap is primarily a mathematical reasoning problem rather than a truncation problem, and that future gains must come from improving answer correctness rather than output length.
...
Diego Otero-Caldwell led infrastructure development, including the inference pipelines (\texttt{run\_baseline.py}, \texttt{phase2\_test.py}), the standardized testing harness (\texttt{testing\_template.py}), and all process documentation. Alex Cojocaru contributed to data analysis, error categorization, and prompt iteration strategy. Shashwat Dudeja contributed to experimental design, result interpretation, and evaluation methodology. All three team members collaborated on the Phase~2 prompt redesign and will share responsibility for fine-tuning and RL experiments in Phases~6--7.
